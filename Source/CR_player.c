...
void CR_AppState_update_player(CR_AppState* state) {
	...
	if (state->playerPlaying) {
		...
		} else if (state->playerAnimation == CR_EPlayerAnim_HURT) {
		...
		} else if (state->playerAnimation == CR_EPlayerAnim_DEATH) {
			if (state->playerFrameIndex == CR_PLAYER_ANIM_LENGTHS[CR_EPlayerAnim_DEATH] - 1) {
